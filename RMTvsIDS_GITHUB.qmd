...
```{r}
df1 <- read_csv(here("radar_ids_features_hr_activity_steps.csv"))
df2 <- read_csv(here("radar_ids_features_sd.csv"))

# Define replacements
replacements <- c("12_month_assessmen_arm_1" = 12,
...
# Use mutate with replace function
df1 <- df1 %>%
  mutate(record_name = ifelse(record_name %in% names(replacements), replacements[record_name], record_name))
...
leepcols <- c("p_id", "record_name", "sleep_day","total_sleep_time_mean", "total_sleep_time_sd", "light_pct_mean", "deep_pct_mean", "REM_pct_mean", "awake_pct_mean", "sleep_onset_mean", "sleep_onset_sd", "sleep_offset_mean", "sleep_offset_sd", "sleep_efficiency_mean", "sleep_efficiency_sd",
"awakenings_mean", "hypersomnia")

df2 <- df2[sleepcols]

df <- merge(df1, df2, by = c("p_id", "record_name"))
```
